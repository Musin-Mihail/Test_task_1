...
public class Spawn : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            GameObject _a = Instantiate(_sphereBig, transform.position, Quaternion.identity, _asteroids.transform);
            _spheresList.Add(_a);
        }
        for (int i = 0; i < 20; i++)
        {
            GameObject _a = Instantiate(_sphereMedium, transform.position, Quaternion.identity, _asteroids.transform);
            _spheresMediumList.Add(_a);
        }
        for (int i = 0; i < 20; i++)
        {
            GameObject _a = Instantiate(_sphereSmall, transform.position, Quaternion.identity, _asteroids.transform);
    ...
    void SpawnSphere(GameObject _sphere)
    {
        ...
        Rigidbody _rigidbodySphere = _sphere.GetComponent<Rigidbody>();
        _rigidbodySphere.velocity = Vector3.zero;
        _sphere.transform.LookAt(Vector3.zero);
        _sphere.SetActive(true);
        _rigidbodySphere.AddForce(_sphere.transform.forward * Random.Range(1.0f, 3.0f), ForceMode.Impulse);
        _spheresLeft++;
    }
}